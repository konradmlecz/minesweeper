        ...
        <BaseLayout>
            <main className={" self-center flex flex-col items-center justify-center flex-1 max-w-screen-lg h-full "}>
                <div
                    ...
                    <h1 className={'p-2 sm:p-4 text-lg sm:text-2xl '}> {t('mainTitle')}</h1>
                    <p className={'pt-2 pl-2 pr-2 sm:pt-4 sm:pl-4 sm:pr-4 pb-2 sm:text-base'}>{t('mainText')}</p>
                    <button
                        className=" self-center text-base sm:text-xl text-white m-4 px-4 py-2 bg-red-500 border-b-2 rounded-full hover:text-red-500 hover:bg-white transition duration-500 ease-in-out">{t('btnPlay')}</button>
                </div>
                <div className={'self-end pt-6,'}>
                    <ReactSVG src="./images/bomb-154456.svg" beforeInjection={(svg) => {
                        svg.classList.add('sm:w-40')
                        svg.classList.add('w-24')
                    }}/>
                </div>
            </main>
        </BaseLayout>
